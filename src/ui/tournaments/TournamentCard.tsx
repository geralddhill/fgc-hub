...
{name: string; slug: string; startTime: Date; profile: Thumbnail; banner: Thumbnail; isOnline: boolean; city?: string | null; state?: string | null;
    ...
    if (expanded) {
        return (<div className="m- bg-mono-200 dark:bg-mono-700 rounded-lg shadow-lg/10 w-80 md:w-144 hover:scale-105 hover:shadow-lg/25 transition duration-250">
            <div onClick={handleClick} className="overflow-hidden p-2 transition-[height] duration-250 md:h-116 h-96 flex flex-col w-full">
                    <ImageWithPlaceholder src={banner.url} width={banner.width} height={banner.height} alt={"Tournament thumbnail"}
                           className="h-70 w-full overflow-hidden rounded-lg margin-auto"/>
                    <div className="mx-2">
                        ...
                        {isOnline ? <p className="font-pui text-mono-600 dark:text-mono-200">Online</p> :
                            <button className="font-pui text-mono-600 dark:text-mono-200 cursor-pointer" onClick={stopPropagation}>
                                <a href={locationURL} target="_blank">{expanded ? address : `${city}, ${state}`}</a>
                            </button>}
                    </div>
    ...
    return (<div className="m-4 bg-mono-200 dark:bg-mono-700 rounded-lg w-80 md:w-144 selectable group">
        <div onClick={handleClick} className="p-2 transition-[height] duration-250 h-24 md:h-36 flex flex-row space-x-3">
